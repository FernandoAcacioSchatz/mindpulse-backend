...
def processar(payload: dict) -> dict:
    ...
    relatorio = supabase.table("relatorio_ia").insert({
        ...
        "projecao_proximo_ciclo": previsao_empresa.get("projecao_proximo_ciclo"),
        "faixa_projetada": previsao_empresa.get("faixa_projetada"),
        "sentimento_geral": resumo_sentimento,
        "classificacao_knn": knn_empresa["classificacao_knn"] if knn_empresa else None,
    ...
    for setor_id, indicadores in indicadores_por_setor.items():
        ...
        supabase.table("indice_setor").insert({
            ...
            "classificacao_knn": knn_setor["classificacao_knn"] if knn_setor else None,
...
def _buscar_respostas(pesquisa_id: str) -> list[dict]:
    resultado = supabase.rpc("obter_respostas_pesquisa", {"p_pesquisa_id": pesquisa_id}).execute()
...
def _buscar_info_tokens(pesquisa_id: str) -> dict:
    ...
    funcionarios = (
        supabase.table("funcionario").select("id, setor_id").in_("id", funcionario_ids).execute().data
        if funcionario_ids else []
    )
...
def _avaliar_qualidade_respostas(respostas: list[dict], tokens_info: dict) -> list[dict]:
...
def _media_anterior_da_categoria(ciclo_anterior_id: str, categoria_id: str) -> float | None:
    linhas = (
...
def _criar_alerta(ciclo_id: str, categoria_id: str | None, tipo: str, descricao: str) -> dict:
